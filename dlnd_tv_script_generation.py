...
def build_nn(cell, rnn_size, input_data, vocab_size, embed_dim):
    ...
    :param cell: RNN cell
    :param rnn_size: Size of rnns ---!WHAT IS THIS FOR?
...
def get_batches(int_text, batch_size, seq_length):
    ...
    """
    # TODO: Implement Function
    n_Characters=batch_size*seq_length
    number_of_batches=len(int_text)//n_Characters
    x=int_text[:number_of_batches*n_Characters]
    y=x[1:]+[x[0]]
    x=np.array(x)
    y=np.array(y)
    ...
    n=0
    for ii in range(0,x.shape[1],seq_length):
        batches[n][0]=x[:,ii:ii+seq_length]
        batches[n][1]=y[:,ii:ii+seq_length]
        n+=1
    return batches
...
def pick_word(probabilities, int_to_vocab):
    ...
    """
    # TODO: Implement Function
    idx_next=np.argmax(probabilities)
    return int_to_vocab[idx_next]
